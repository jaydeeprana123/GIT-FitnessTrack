...
class HolidayController extends GetxController {
  ...
  getHolidayListAPI() async {
    ...
    String url = urlBase + urlHolidayList;
    ...
    request.body = json.encode({
      "id": loginResponseModel.value.data?[0].id??""
    });
    ...
    if (response.statusCode == 200) {
      await response.stream.bytesToString().then((valueData) async {
        ...
        HolidayListModel holidayListModel = HolidayListModel.fromJson(userModel);
        holidayList.value = holidayListModel.data ?? [];
      });
      ...
      Get.dialog(
        NoInternetDialog(
          onRetry: () {
            ...
            getHolidayListAPI(); // Retry API call
          },
        ),
